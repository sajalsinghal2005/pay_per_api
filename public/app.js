...
const initSignup = () => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      setLoading(button, false, 'Signup');
    }
  });
};
    ...
    try {
      ...
      weatherResult.innerHTML = `
        <div class="card card-soft p-4 mt-4">
          <h6 class="fw-semibold mb-2">${data.city}</h6>
          <p class="mb-1 text-muted">${data.weather}</p>
          <p class="mb-1 fw-semibold">${data.temperature}°C</p>
          <p class="text-muted small mb-0">Credits left: ${data.remainingCredits}</p>
        </div>
      `;
      ...
      setLoading(button, false, 'Get Weather');
    }
  });
};
